...
from late_fusion.export_3D_to_2D import project_3D_to_2D
...
class Late_fusion_dataset(Dataset):
    ...
    def get_2d_file(self, sample, ref_chan):
        ...
        cam_path, _, ref_cam_intrinsic = self.nusc.get_sample_data(cam_token)
        cam_path = cam_path.split('/')[-1]
        ...
        top_predictions = middle_predictions[torch.where(middle_predictions[:, 4] >= 0.3)]  # not do filter
    ...
    def generate_input(self):
        ...
        for sample in tqdm(self.nusc.sample):
            ...
            for id, cam_channel in enumerate(self.cam_channels):
                ...
                lidar_2D_bbox, cam_frame = project_3D_to_2D(self.nusc, boxes_3d_global, sample_data_token, lidar_2D_bbox, cam_frame, channel_id)  # [2d_boxes],[channel_id]
                # get camera_box
                Cam_box = self.get_2d_file(sample, cam_channel)  # [[x1,y1,x2,y2,score]..]
    ...
    def train_stage_2(self, predict_3d, Cam_boxes):
        ...
        dis_to_lidar = torch.norm(box3d[:, :2], p=2, dim=1, keepdim=True).detach().cpu().numpy() / 73
        ...
        for cam_channel in self.cam_channels:
            ...
            box_2d_detector = torch.zeros([200, 4], dtype=torch.float32).cuda()
            box_2d_scores = torch.zeros([200, 1], dtype=torch.float32).cuda()
...
def build_stage2_training(boxes, Box_2d_detector, criterion, scores_3d, Box_2d_scores, dis_to_lidar_3d, overlaps, tensor_index, cam_frame):
    ...
    N = boxes.shape[0]  # 1000  3d detector 2d bbox
    K = Box_2d_detector.shape[0] / 6  # 200  2d detector bbox
    max_num = 900000
    ind = 0
    ind_max = ind
    for n in range(N):
        # get the camera results from the corresponding camera channel
        cam_channel_id = (cam_frame[n,0] - 1)*200
        scores_2d = Box_2d_scores[cam_channel_id:(cam_channel_id + 200), :]
        query_boxes = Box_2d_detector[cam_channel_id:(cam_channel_id + 200), :]

        for k in range(K):
            if cam_channel_id >= 0:  # 3d proposals with no 2d projection
