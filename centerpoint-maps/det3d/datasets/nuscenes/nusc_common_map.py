...
general_to_detection = {
    "human.pedestrian.adult": "ignore",
    "human.pedestrian.child": "ignore",
    "human.pedestrian.wheelchair": "ignore",
    "human.pedestrian.stroller": "ignore",
    "human.pedestrian.personal_mobility": "ignore",
    "human.pedestrian.police_officer": "ignore",
    "human.pedestrian.construction_worker": "ignore",
    "animal": "ignore",
    "vehicle.car": "car",
...
def get_sample_map_data_path(nusc, sample_data_token: str):
    sd_record = nusc.get("sample_data", sample_data_token)
    timestamp = sd_record['timestamp']

    path = osp.join(nusc.dataroot,'maps_generated','HDMAP')

    map_data_path = osp.join(path, sample_data_token) + '__HDMAP' + '.png'
...
def get_sample_data(
    ...
    :return: (data_path, map_data_path, boxes, camera_intrinsic <np.array: 3, 3>)
    """
    ...
    data_path = nusc.get_sample_data_path(sample_data_token)
    map_data_path = get_sample_map_data_path(nusc, sample_data_token)
    ...
    return data_path, map_data_path, box_list, cam_intrinsic
...
def get_sample_ground_plane(root_path, version):
    ...
    for sample in tqdm(nusc.sample):
        ...
        lidar_path, _, _, _ = get_sample_data(nusc, sd_token)
...
def _fill_trainval_infos(nusc, nusc_exp, train_scenes, val_scenes, test=False, nsweeps=10, filter_zero=True):
    from nuscenes.utils.geometry_utils import transform_matrix
    ...
    for sample in tqdm(nusc.sample):
        ...
        ref_lidar_path, ref_map_path, ref_boxes, _ = get_sample_data(nusc, ref_sd_token)
        ...
        while len(sweeps) < nsweeps - 1:
            if curr_sd_rec["prev"] == "":
                ...
                # map_path = get_sample_map_data_path(nusc, curr_sd_rec["token"])
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", nsweeps=10, filter_zero=True):
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_exp, train_scenes, val_scenes, test, nsweeps=nsweeps, filter_zero=filter_zero
    )
